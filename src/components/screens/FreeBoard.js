...
class FreeBoard extends React.Component {
    ...
    componentDidMount = async () => {
        try {
            await axios.post("/api/getFreeboardData").then(
        ...
        return (
            <WholeWrapper>
                ...
                {datum ? (
                    datum.map((data, idx) => {
                        return (
                            <Wrapper key={data.refKey} width="960px" height="25px" direction="row" isData={true} onClick={() => this.props.history.push("/detail/anyID")}>
